...
layui.define(['layer', 'form'], function(exports){
    ...
    ,toolActive = function(iframeWin, editor, set){
      ...
      ,toolEvent = {
        ...
        //错别字检查
        ,cuobiezi: function(range){
            //清除格式
            var editText = body[0].innerHTML;
            ...
            editText = editText.replace(re, '');console.log(editText);
            //普通字符转换成转意符
            var text = editText.replace(/[<>&"'“”]/g,function(c){return {'<':'&lt;','>':'&gt;','&':'&amp;','"':'&quot;','\'':'&#39;','“':'&ldquo;','”':'&rdquo;'}[c];});
            //回车转为br标签
            text = text.replace(/\r?\n/g,"<br />");
            ...
            $.ajax({
                ...
                success: function(json_obj){
                    ...
                    pos_arr.sort(function(a,b){return a-b;});

                    var new_conent_raw = "";
                    var _tmp_start_pos = 0;
                    var arrayLength = pos_arr.length;
                    for (var i = 0; i < arrayLength; i++) {
                        if( pos_arr[i] >= text.length ){
                            continue;
                        }
                        var str_tmp = text.substring(_tmp_start_pos, pos_arr[i])
                        ...
                        if(_tmp_obj.ReviewWords){
                            ...
                            new_conent_raw = new_conent_raw + "<span style=\"color:#FF5722\" onmouseover=\"parent.errorText('"+ _tmp_obj.Tips +"')\"><strong>"+ _tmp_obj.Error +"</strong></span>"	
                        }
                        _tmp_start_pos = pos_arr[i] + _tmp_sub_len;
                    }
                    str_tmp = text.substring(_tmp_start_pos, text.length)
                    new_conent_raw = new_conent_raw + str_tmp;
                    //转意符换成普通字符
                    var arrEntities={'lt':'<','gt':'>','nbsp':' ','amp':'&','quot':'"','#39':'\'','ldquo':'“','rdquo':'”'};
                    new_conent_raw = new_conent_raw.replace(/&(lt|gt|nbsp|amp|quot|#39|ldquo|rdquo);/ig,function(all,t){return arrEntities[t];});
                    console.log(new_conent_raw);
                    body[0].innerHTML = new_conent_raw;
                    layer.msg('错别字检测完成！', {icon: 6});  
                },
            });
        }
        //清除标记格式
        ,clear: function(){
            ...
            re = new RegExp(find, 'g');
            editText = editText.replace(re, '');
            body[0].innerHTML = editText;
            layer.msg('清除标记格式完成！', {icon: 6});
        }
        //帮助
        ,help: function(){
          layer.open({
    ...
    ,tools = {
      ...
      ,cuobiezi: '<i class="layui-icon layedit-tool-cuobiezi" title="错别字检查" layedit-event="cuobiezi" style="font-size:18px!important;">&#xe6b2;</i>'
      ,clear: '<i class="layui-icon layedit-tool-clear" title="清除标记格式" layedit-event="clear" style="font-size:18px!important;">&#xe639;</i>'
